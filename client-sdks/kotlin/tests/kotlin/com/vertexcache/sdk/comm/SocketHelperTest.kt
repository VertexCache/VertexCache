package com.vertexcache.sdk.comm

import com.vertexcache.sdk.model.ClientOption
import com.vertexcache.sdk.model.VertexCacheSdkException
import org.junit.jupiter.api.*
import java.io.IOException
import java.net.ServerSocket
import java.net.Socket
import kotlin.test.assertEquals
import kotlin.test.assertNotNull
import kotlin.test.assertTrue
import kotlin.test.assertFailsWith
import kotlin.test.assertFalse

@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class SocketHelperTest {

    private val mockPort = 18888
    private val unusedPort = 65534
    private val liveTLSPort = 50505
    private val connectTimeout = 1000
    private val readTimeout = 1000
    private var serverRunning = true

    private val validPemCert = """
        -----BEGIN CERTIFICATE-----
        
        qwwA44GZv7zAa89WHNpbIMAA8keexZkPzJBIQNSKy2d9dhcP
        -----END CERTIFICATE-----
    """.trimIndent()

    @BeforeAll
    fun startMockServer() {
        Thread {
            try {
                ServerSocket(mockPort).use { serverSocket ->
                    while (serverRunning) {
                        try {
                            val socket = serverSocket.accept()
                            socket.close()
                        } catch (_: IOException) {}
                    }
                }
            } catch (_: IOException) {}
        }.apply {
            isDaemon = true
            start()
        }
    }

    @AfterAll
    fun stopMockServer() {
        serverRunning = false
        try {
            Socket("localhost", mockPort).close()
        } catch (_: IOException) {}
    }

    @Test
    fun createNonSecureSocket_shouldSucceed() {
        val opt = ClientOption().apply {
            serverHost = "localhost"
            serverPort = mockPort
            connectTimeout = 1000
            readTimeout = 1000
        }
        val socket = SocketHelper.createSocketNonTLS(opt)
        assertNotNull(socket)
        assertTrue(socket.isConnected)
        socket.close()
    }

    @Test
    fun createNonSecureSocket_shouldFailIfPortClosed() {
        val opt = ClientOption().apply {
            serverHost = "localhost"
            serverPort = unusedPort
            connectTimeout = 500
            readTimeout = 500
        }
        val ex = assertFailsWith<VertexCacheSdkException> {
            SocketHelper.createSocketNonTLS(opt)
        }
        assertEquals("Failed to create Non Secure Socket", ex.message)
    }

    @Test
    fun createNonSecureSocket_shouldFailOnTimeout() {
        val opt = ClientOption().apply {
            serverHost = "10.255.255.1"
            serverPort = 12345
            connectTimeout = 300
            readTimeout = 500
        }
        val ex = assertFailsWith<VertexCacheSdkException> {
            SocketHelper.createSocketNonTLS(opt)
        }
        assertEquals("Failed to create Non Secure Socket", ex.message)
    }

    @Test
    fun createSecureSocket_shouldFailDueToMissingTLSContext() {
        val opt = ClientOption().apply {
            serverHost = "localhost"
            serverPort = mockPort
            connectTimeout = 1000
            readTimeout = 1000
            verifyCertificate = true
            tlsCertificate = null
        }
        val ex = assertFailsWith<VertexCacheSdkException> {
            SocketHelper.createSocketTLS(opt)
        }
        assertEquals("Failed to create Secure Socket", ex.message)
    }

    @Test
    fun createSecureSocket_shouldFailWithBadCertificate() {
        val opt = ClientOption().apply {
            serverHost = "localhost"
            serverPort = mockPort
            connectTimeout = 1000
            readTimeout = 1000
            verifyCertificate = true
            tlsCertificate = "not a cert"
        }
        val ex = assertFailsWith<VertexCacheSdkException> {
            SocketHelper.createSocketTLS(opt)
        }
        assertEquals("Failed to create Secure Socket", ex.message)
    }

    @Test
    fun createSecureSocket_shouldSucceedWithLiveServer() {
        Assumptions.assumeTrue(System.getenv("ENABLE_LIVE_TLS_TESTS") == "true")

        val opt = ClientOption().apply {
            serverHost = "localhost"
            serverPort = liveTLSPort
            connectTimeout = 1000
            readTimeout = 1000
            verifyCertificate = true
            tlsCertificate = validPemCert
        }

        val socket = SocketHelper.createSocketTLS(opt)
        assertNotNull(socket)
        assertTrue(socket.isConnected)
        socket.close()
    }
}