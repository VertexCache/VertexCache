...
pub struct GetSecondaryIdxOneCommand {
    key: String,
    value: Option<String>,
    pub success: bool,
...
impl Command for GetSecondaryIdxOneCommand {
    fn execute(&mut self, connector: &mut ClientConnector) {
        let command_str = format!("GETIDX1 {}", self.key);
        ...
        match connector.send(&command_str) {
            Ok(response) => {
                ...
                if trimmed.eq_ignore_ascii_case("(nil)") {
                    ...
                    self.success = false;
                    self.status_message = format!("GETIDX1 failed: {}", trimmed);
                    self.value = None;
                } else {
                    self.success = true;
            ...
            Err(e) => {
                self.success = false;
                self.status_message = format!("GETIDX1 failed: {}", e);
                self.value = None;
            }
        }
    }
