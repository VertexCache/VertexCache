...
defmodule VertexCacheSdk.Command.Impl.GetSecondaryIdxTwoCommand do
  @moduledoc """
  Handles the GET Secondary Idx (idx2) command in VertexCache.

  Retrieves the value for a given key from the cache using a tertiary index.
  ...
  def new(key) when is_binary(key) do
    if String.trim(key) == "" do
      raise VertexCacheSdkException, message: "GET By Secondary Index (idx2) command requires a non-empty key"
    else
      %__MODULE__{key: key}
    end
  end
  ...
  def new(_),
      do: raise VertexCacheSdkException,
          message: "GET By Secondary Index (idx2) command requires a non-empty key"

  @spec execute(t, ClientConnector.t()) :: GetResult.t()
  def execute(%__MODULE__{key: key}, connector) do
    CommandBase.execute("GETIDX2 " <> key, connector, &parse_response/1)
    |> to_get_result()
  end
