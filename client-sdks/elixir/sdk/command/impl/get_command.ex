...
defmodule VertexCacheSdk.Command.Impl.GetCommand do
  @moduledoc """
  ...
  def new(key) when is_binary(key) do
    if String.trim(key) == "" do
      raise VertexCacheSdkException, message: "GET command requires a non-empty key"
    else
      %__MODULE__{key: key}
    end
  end
  ...
  def new(_),
      do: raise VertexCacheSdkException, message: "GET command requires a non-empty key"

  @spec execute(t, ClientConnector.t()) :: GetResult.t()
  def execute(%__MODULE__{key: key}, connector) do
    CommandBase.execute("GET " <> key, connector, fn resp -> parse_response(resp) end)
    |> to_get_result()
  end
